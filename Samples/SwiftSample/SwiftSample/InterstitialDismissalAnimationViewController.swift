...
class InterstitialDismissalAnimationViewController: UIViewController, SASAdViewDelegate {
	...
	func createInterstitial() {
		...
		if let actualInterstitial = interstitial {
			...
			actualInterstitial.translatesAutoresizingMaskIntoConstraints = false
			
			// Then we add constraints so that the interstitial stays below the navigation bar for all sizes and orientations.
			let views: [String : AnyObject] = ["v": actualInterstitial]
            navigationController?.view.addConstraints(NSLayoutConstraint.constraintsWithVisualFormat("V:|[v]|", options: NSLayoutFormatOptions.DirectionLeadingToTrailing, metrics: nil, views: views))
			navigationController?.view.addConstraints(NSLayoutConstraint.constraintsWithVisualFormat("|[v]|", options: NSLayoutFormatOptions.DirectionLeadingToTrailing, metrics: nil, views: views))
            
          /*  navigationController?.view.addConstraint(NSLayoutConstraint.constraintsWithVisualFormat(<#T##format: String##String#>, options: NSLayoutFormatOptions, metrics: <#T##[String : AnyObject]?#>, views: <#T##[String : AnyObject]#>))*/
		}
	}
